...
taux_technique = st.sidebar.number_input("Taux d'intérêt technique (%)", min_value=0.0, step=0.01, value=1.05)
taux_equilibree = st.sidebar.number_input("Taux profil équilibré (%)", min_value=0.0, step=0.01, value=3.0)
taux_revenu = st.sidebar.number_input("Taux profil revenu (%)", min_value=0.0, step=0.01, value=5.0)
taux_action = st.sidebar.number_input("Taux profil actions (%)", min_value=0.0, step=0.01, value=7.0)
...
for annee in annees:
    ...
    # Calcul des intérêts
    cumul_technique = cumul_technique * (1 + taux_technique / 100) + prime_epargne
    profil_equilibree = cumul_total * (1 + taux_equilibree / 100)
    profil_revenu = cumul_total * (1 + taux_revenu / 100)
    profil_actions = cumul_total * (1 + taux_action / 100)
    ...
    data["Prime épargne avec intérêt"].append(cumul_technique)
    data["Profil équilibré"].append(profil_equilibree)
    data["Profil revenu"].append(profil_revenu)
    data["Profil actions"].append(profil_actions)
    data["Coût d'opportunité"].append(profil_actions - cumul_technique)
